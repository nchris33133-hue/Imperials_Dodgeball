...
function signupConfirmation({ name, type }) {
  const isNewsletter = type === 'newsletter';

  const subject = isNewsletter
    ? 'Newsletter-Anmeldung bestätigt / Newsletter Subscription Confirmed'
    : 'Willkommen bei den Vienna Imperials! / Welcome to the Vienna Imperials!';

  const content = isNewsletter
    ? `
      <h2 style="margin:0 0 16px;font-size:20px;color:#F0B830;font-weight:800;letter-spacing:1px;">NEWSLETTER BESTÄTIGT / CONFIRMED</h2>
      <p style="margin:0 0 16px;font-size:15px;color:rgba(244,247,255,0.8);line-height:1.6;">
        ${name ? `Hallo ${name},` : 'Hallo,'}<br/><br/>
        Danke für deine Anmeldung zum Vienna Imperials Newsletter! Du erhältst Updates zu Events, Turnieren und Club-Neuigkeiten.<br/><br/>
        Thanks for subscribing to the Vienna Imperials newsletter! You'll receive updates about events, tournaments, and club news.
      </p>
      <p style="margin:16px 0 0;font-size:12px;color:rgba(244,247,255,0.35);line-height:1.5;">

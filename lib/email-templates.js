...
function contactMessage({ name, email, message }) {
  const subject = `Neue Nachricht / New Message: ${name}`;

  const escapedMessage = message.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/\n/g, '<br/>');

  const content = `
      <h2 style="margin:0 0 16px;font-size:20px;color:#F0B830;font-weight:800;letter-spacing:1px;">NEUE NACHRICHT / NEW MESSAGE</h2>
      <table width="100%" cellpadding="0" cellspacing="0" style="background:rgba(12,27,58,0.6);border:1px solid rgba(53,103,192,0.2);border-radius:8px;margin:0 0 24px;">
        <tr><td style="padding:20px;">
          <div style="font-size:12px;font-weight:700;letter-spacing:2px;color:#F0B830;text-transform:uppercase;margin-bottom:4px;">Von / From</div>
          <div style="font-size:16px;font-weight:700;color:#F4F7FF;margin-bottom:2px;">${name}</div>
          <div style="font-size:14px;color:rgba(244,247,255,0.6);margin-bottom:16px;">${email}</div>
          <div style="font-size:12px;font-weight:700;letter-spacing:2px;color:#F0B830;text-transform:uppercase;margin-bottom:4px;">Nachricht / Message</div>
          <div style="font-size:15px;color:rgba(244,247,255,0.85);line-height:1.6;">${escapedMessage}</div>
        </td></tr>
      </table>
      <p style="margin:0;font-size:12px;color:rgba(244,247,255,0.35);line-height:1.5;">
